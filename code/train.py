...
class Trainer:
    def __init__(self, total_cls):
        ...
        self.optimizer = AdamW(
            optimizer_grouped_parameters, lr=FLAGS.lr, eps=FLAGS.adam_epsilon
        )
    ...
    def train(self, batch_size, epoches, max_size):
        ...
        print("load data……")
        data_x_train, data_mask_train, data_y_train = load.load_data_bert(
            FLAGS.data_path
        )
        data_x_test, data_mask_test, data_y_test = load.load_data_bert(
            FLAGS.data_path.replace("train", "test")
        )
        ...
        for inc_i in range(FLAGS.task_num):
            ...
            train_dataloader = D.DataLoader(
                train_dataset, batch_size, True, num_workers=5
            )
            test_dataset = D.TensorDataset(test_xss, test_maskss, test_yss)
            test_dataloader = D.DataLoader(
                test_dataset, batch_size, False, num_workers=5
            )
    ...
    def stage1(self, train_dataloader, criterion):
        ...
        for train_X, train_mask, train_Y in train_dataloader:
            ...
            print("p111", p.view(-1, self.seen_cls + 1).shape)
            print("p222", train_Y.view(-1).shape)
    ...
    def stage1_distill(self, train_dataloader, criterion, distill_criterion):
        ...
        T = 2
        for train_X, train_mask, train_Y in train_dataloader:
            print(train_X.shape, train_mask.shape, train_Y.shape)
            ...
            normalized_output = F.normalize(
                output.view(-1, output.size()[2]), p=2, dim=1
            )

            p = logits[:, :, : self.seen_cls + 1]
            with torch.no_grad():
                pre_p, pre_output = self.previous_model(train_X, train_mask)
                normalized_pre_out = F.normalize(
                    pre_output.view(-1, pre_output.size()[2]), p=2, dim=1
                )
                pre_p = F.softmax(pre_p[:, :, : self.seen_cls] / T, dim=2)
                pre_p = pre_p.view(-1, self.seen_cls)
            logp = F.log_softmax(p[:, :, : self.seen_cls] / T, dim=2).view(
                -1, self.seen_cls
            )
            loss_soft_target = -torch.mean(torch.sum(pre_p * logp, dim=1))
            # loss_hard_target = criterion(
            #     p.view(-1, self.seen_cls + 1), train_Y.view(-1)
            # )
            distill_loss = distill_criterion(
                normalized_output,
                normalized_pre_out,
                torch.ones(train_X.size(0) * train_X.size(1)),
            )

            print("normalized_output", normalized_output.shape)
            print("normalized_pre_out", normalized_pre_out.shape)
            print("torch.ones", torch.ones(train_X.size(0) * train_X.size(1)).shape)

            loss = loss_soft_target * 0.8 + 0.7 * distill_loss

            print("****____*****", loss_soft_target, distill_loss)
            loss.backward()
            self.optimizer.step()
            self.model.zero_grad()
            total_loss += loss.item()
